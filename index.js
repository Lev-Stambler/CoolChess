...
function dropTD(info, event, elem, player) {
  ...
  const appenderParentNumb = parseInt(appender.parentElement.getAttribute('class') ||
    appender.parentElement.getAttribute('class')[0], 16)
  console.log("appenderParentNumb", appenderParentNumb)

  const hoverParentNumb = parseInt(elem.getAttribute('class') ||
...
function drop(info, event, elem, player) {
  const oppPlayer = player === 'p2' ? 'p1' : 'p2'
  console.log('oppPlayer', oppPlayer)
  event.preventDefault()
  var data = event.dataTransfer.getData('text')
  console.log(elem)
  ...
  const appenderParentNumb = parseInt(appender.parentElement.getAttribute('class') || appender.parentElement.getAttribute('class')[0], 16)

  const hoverParentNumb = parseInt(elem.parentElement.getAttribute('class') ||
...
function isLegalMove(pieceIndex, currPos, futurePos, options) {
  ...
  console.log(pieceIndex, currPos, futurePos)
